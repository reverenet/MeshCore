...
# examples/configs/sample.conf for the network profile, tools/flash_erase/README.md for erasing.
...
ARGUMENTS
 ...
 build and flash
  ...
  BLE_PIN       6 digits, replacing the pin from the variant files. Boards with a screen
                ...
                BLE are unaffected. Normally left to the pin file rather than passed here,
                for the same reason the keys are: a command line is remembered by the
                shell, by CI, and by whoever it gets pasted to.
  BLE_PIN_FILE  where the pin is read from, under REVERENET_HOME. One line of 6 digits,
                with # comments ignored. Not generated by 'make keys' - the pin is chosen,
                not random - and, like the key files, a missing default file is not an
                error. KEYS_DIR moves it along with the keys.
 ...
 keys - the shared secrets of the network
  ...
  TRACKING_KEY_FILE, KEYS_DIR
                where the key is read from, keys/tracking.key under REVERENET_HOME. A missing default
                ...
                by side.
  CHANNELS_KEY_FILE
                one key per channel named in CHANNELS, keys/channels.key under REVERENET_HOME, as
...
EXAMPLES
  ...
  echo 456123 > ~/.reverenet/keys/ble.pin   # every build uses it, and none of them say so
...
# build.sh reads this from the environment. BLE_PIN_CODE is exported further down, once
# the key files have been read - the pin can come from one of those, and exporting here
# would capture the value before anything had looked.
...
# ------------------------------------------------------------------- where yours live
#
# A network's profile and its keys are not the firmware's, and they do not belong in a
# checkout of it. A profile names the radio settings, the regions and the channels of a
# real network; the keys next to it ARE that network. Both outlive any one clone, both
# want to be shared between clones, and neither should ever be one 'git add .' away from
# being published.
#
...
#
# Point REVERENET_HOME somewhere else to keep several networks side by side, or to build
# from a directory somebody handed you without copying anything into the repo.
#
# examples/configs/sample.conf, which IS in the tree, lists every setting a profile can
# carry. It is a menu to copy from, and it names no network.
REVERENET_HOME ?= $(HOME)/.reverenet
...
ifneq ($(strip $(CONFIG)),)
  ifeq ($(wildcard $(CONFIG)),)
    # Naming a file is a statement that it exists; falling back on the default is not.
    # A missing default builds the stock per-variant settings, the same as CONFIG= - but
    # NOT quietly, because a node built on stock radio settings does not hear the network
    # it was meant for and nothing about it says why.
    ifeq ($(CONFIG_IS_DEFAULT),)
      $(error CONFIG=$(CONFIG) does not exist)
    endif
    $(warning no profile at $(CONFIG) - building the stock per-variant settings. Set \
              REVERENET_HOME or CONFIG to build for a network, or CONFIG= to say you \
              meant the stock ones)
    CONFIG :=
  endif
endif
...
KEYS_DIR ?= $(REVERENET_HOME)/keys
TRACKING_KEY_FILE ?= $(KEYS_DIR)/tracking.key

# The BLE pairing pin belongs here with the keys rather than on a command line: it is a
# per-network secret with a six digit keyspace, and a command line ends up in shell
# history, in a CI log, and in the diff when someone pastes the command into a comment.
# Same file format as a key - one line, # comments ignored - and the same rule about a
...
ifeq ($$($(1)),)
  ifneq ($$(wildcard $$($(1)_FILE)),)
    ...
    ifeq ($$($(1)),)
      $$(error $$($(1)_FILE) holds no value - every line is blank or a # comment)
    endif
  else ifneq ($$(filter command line environment,$$(origin $(1)_FILE)),)
    $$(error $(1)_FILE=$$($(1)_FILE) does not exist$(if $(2), - $(2)))
  endif
else
...
$(eval $(call resolve_key_file,TRACKING_KEY,run 'make keys' to generate it))
$(eval $(call resolve_key_file,BLE_PIN,put six digits in it))

# Checked here rather than left to build.sh so a malformed pin file is caught before a
# build starts. The value itself is never printed: this output gets pasted into issues.
ifneq ($(BLE_PIN),)
  ifneq ($(shell printf %s '$(BLE_PIN)' | grep -qE '^[0-9]{6}$$' || echo bad),)
    $(error BLE_PIN must be exactly 6 digits - check $(BLE_PIN_SOURCE))
  endif
endif

# now that the file has been read, hand it to build.sh
...
# The keys and the profiles moved out of the tree, and a checkout that still has the old
# directories would otherwise build without either: no tracking key compiles position
# reporting out, and no profile builds stock radio settings, both in silence. Say so once
# rather than shipping a node that cannot hear its own network.
ifeq ($(KEYS_DIR),$(REVERENET_HOME)/keys)
  ifneq ($(wildcard keys/.)$(wildcard configs/.),)
    $(error keys/ and configs/ now live under REVERENET_HOME, which is $(REVERENET_HOME). \
            Move them there - or pass KEYS_DIR and CONFIG to build from where they are - \
            and delete what is left in the checkout so it cannot be committed)
  endif
endif
